...
void QTextStreamPrivate::flushWriteBuffer()
{
    ...
    // flush the file
#if !defined(QT_NO_FILEENGINE)
#if !defined(QT_NO_QOBJECT)
    QFileDevice *file = qobject_cast<QFileDevice *>(device);
#else
    QFileDevice *file = deleteDevice ? static_cast<QFileDevice *>(device) : 0;
#endif
    ...
    : d_ptr(new QTextStreamPrivate(this))
{
    ...
    d->status = Ok;
#else
    qCritical("QTextStream::QTextStream(FILE *fileHandle, openMode) not supported with QT_NO_FILEENGINE");
    Q_D(QTextStream);
    d->status = Ok;
#endif
}
